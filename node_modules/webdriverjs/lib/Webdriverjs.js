...
WebdriverJs.prototype.createRequest = function(requestOptions, data, callback){
  ...
  //dont do agent pooling 
  fullRequestOptions.agent = false;
  ...
  return function(response){
    ...
    response.on('end', function(){
      if (self.logLevel === 'verbose'){
        self.log(colors.navy + "RES DATA\t\t " + colors.reset + data);
      }
      if (options){
        if (options.setSessionId){
          try{
            ...
            self.log(colors.magenta + "SET SESSION ID " + colors.reset, sessionId);
          }catch(err){
            self.log(colors.red + "COULDNT GET A SESSION ID" + colors.reset);
            return callback(new Error('Could\'nt get a session id'));
          }
        }
      }
...
WebdriverJs.prototype.terminate = function(cb){
  //clear down the queue and run the callback
  this.currentQueueItem.isDone = true;
  this.currentQueueItem.done();
  this.end(function(){
    cb();
  })
}
