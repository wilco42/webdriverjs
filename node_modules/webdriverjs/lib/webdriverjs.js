...
var WebdriverJs = function(options)//host, port, username, pass) 
{
	...
	var QueueItem = function(commandName, method, scope, args)
	{
		...
		for(var i = 0, ii = args.length; i < ii; i++)
		{
			...
			{
				hasCallback = true;
				this.args.push((function(method, methodName)
					{
...
WebdriverJs.prototype.getResults = function() {
    var results = this.results.join("\n");
    results += "\n";
    fs.writeFile('results.xml', results, function(err) {});
    return this;
};
...
// make JSCoverage report in a CI-compatible format
WebdriverJs.prototype.makeJsCoverageReport = function(data, files) {
    var totalLines = 0;
    var count = 0;
    for(var j in files) {
        for(var i in data.value[files[j]]) {
            if (data.value[files[j]][i] !== null ) {
                totalLines++;
                if (data.value[files[j]][i] > 0) {
                    count++;
                }
            }
        }
        var codeCoverage = parseInt(count/totalLines*100);
        console.log('Code Coverage: ' + codeCoverage + '%');
    }
};

// setup webdriverjs
exports.parseParams = function(options) {
    var args = options.slice(2);
    var opts = {};
    if (args.length != 3) {
        opts = {
            platform: 'WINDOWS',
